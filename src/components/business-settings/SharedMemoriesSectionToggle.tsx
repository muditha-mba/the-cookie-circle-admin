...
export function SharedMemoriesSectionToggle() {
  const queryClient = useQueryClient();
  const [error, setError] = useState<string | null>(null);

  const { data, isLoading, isError } = useQuery({
    queryKey: ["shared-memories-section-settings"],
    queryFn: () => sharedMemoriesApi.getSectionSettings(),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (updated) => {
      setError(null);
      queryClient.setQueryData(["shared-memories-section-settings"], updated);
    },
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            Show or hide the Shared Memories carousel on the client home page.
          </p>
        </div>
